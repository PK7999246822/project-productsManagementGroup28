...
const getUserProfile = async function (req, res) {
    try {
        ...
        if (userIdFromToken != userProfile._id) {
            return res.status(403).send({ status: false, message: 'Unauthorized access' })
        }

        return res.status(200).send({ status: true, message: 'Profile found successfully', data: userProfile })
    }
    catch (error) {
        return res.status(500).send({ status: false, message: error.message })
    }
...
const updateProfile = async function (req, res) {
    try {
        ...
        if (!validator.isValidRequestBody(updateBody)) {
            return res.status(400).send({ status: false, message: 'Please provide update body to update' })
        }
        ...
        if (tempPassword) {
            ...
            if (!(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,15}$/.test(tempPassword.trim()))) {
                return res.status(400).send({ status: false, message: "please provide atleast one uppercase letter ,one lowercase, one character and one number to update " })
           }
